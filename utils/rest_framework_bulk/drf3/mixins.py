...
class GetObjectsMixin:
    def get_objects(self, request, *args, **kwargs):
        ...
        [self.check_object_permissions(request, obj) for obj in queryset]
        return queryset
...
class BulkUpdateModelMixin(GetObjectsMixin):
    """
    ...
    def bulk_update(self, request, *args, **kwargs):
        ...
        queryset = self.get_objects(request, *args, **kwargs)
...
class BulkDestroyModelMixin(GetObjectsMixin):
    """
    ...
    def bulk_destroy(self, request, *args, **kwargs):
        """
        delete:
        删除多个id请使用路由："/manage/users/?id__in=8,9"
        即删除id为8和9的用户
        """
        qs = self.get_queryset()

        filtered = self.get_objects(request, queryset=qs, *args, **kwargs)
